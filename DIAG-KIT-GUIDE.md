...
> [HWiNFO64](#hwinfo64) — Deep hardware sensors
> [CPU-Z](#cpu-z) — CPU · RAM · Board ID
> [Speccy](#speccy) — Quick hardware summary

> [!tip] 🌐 02 — Network
> [Angry IP Scanner](#angry-ip-scanner) — Scan all devices on LAN
> [CurrPorts](#currports) — Live connections on this PC
> [Wireshark Portable](#wireshark-portable) — Deep packet capture
> [ManageEngine NetFlow](#manageengine-netflow-analyzer-free) — Bandwidth monitor (installer only)

> [!warning] 💾 03 — Storage
> [CrystalDiskInfo](#crystaldiskinfo) — SMART disk health
> [WinDirStat](#windirstat) — Visual space map

> [!example] 🔬 04 — Windows Integrity (Sysinternals)
> [Autoruns](#autoruns) — Everything that runs at startup
> [Process Explorer](#process-explorer) — Advanced Task Manager
> [Process Monitor](#process-monitor) — File · Registry · Network events
> [TCPView](#tcpview) — Live network connections
> [RAMMap](#rammap) — Memory usage breakdown
> [VMMap](#vmmap) — Per-process memory map
> [Sigcheck](#sigcheck) — File signature + VirusTotal
> [AccessChk and AccessEnum](#accesschk-and-accessenum) — Permissions audit
> [Handle](#handle) — "File is in use" solver
> [Strings](#strings) — Extract text from binaries
> [Sysmon](#sysmon) — Persistent system event logging
> [PsTools Suite](#pstools-suite) — Remote management
> [DiskView and Diskmon](#diskview-and-diskmon) — Disk sector map + activity
> [BGInfo](#bginfo) — System info on desktop
> [ProcDump](#procdump) — Capture crash dumps
> [RDCMan](#rdcman) — Remote Desktop manager
> [ADExplorer](#adexplorer) — Active Directory browser
> [ZoomIt](#zoomit) — Screen zoom + annotation
> [Other Sysinternals Tools](#other-sysinternals-tools)

> [!danger] 🛡️ 05 — Malware
> [AdwCleaner](#adwcleaner) — Adware · PUPs · Browser hijacks
> [ESET Online Scanner](#eset-online-scanner) — Full deep AV scan

> [!abstract] 🥾 07 — Boot Tools
> [Medicat](#medicat) — Bootable rescue toolkit
...
|---|---|---|
| Machine slow, fan screaming | [HWiNFO64](#hwinfo64) | 01 |
| "What hardware does this have?" | [CPU-Z](#cpu-z) or [Speccy](#speccy) | 01 |
| Can't reach internet | [Angry IP Scanner](#angry-ip-scanner) → [CurrPorts](#currports) | 02 |
| Suspicious outbound traffic | [CurrPorts](#currports) → [Wireshark](#wireshark-portable) | 02 |
| Disk is full, don't know why | [WinDirStat](#windirstat) | 03 |
| Disk making noise / S.M.A.R.T. warning | [CrystalDiskInfo](#crystaldiskinfo) | 03 |
| Something weird starts at boot | [Autoruns](#autoruns) | 04 |
| Unknown process eating CPU/RAM | [Process Explorer](#process-explorer) | 04 |
| "A file keeps getting modified" | [Process Monitor](#process-monitor) | 04 |
| Live connections look suspicious | [TCPView](#tcpview) | 04 |
| Machine feels slow despite enough RAM | [RAMMap](#rammap) | 04 |
| File won't delete — "in use" error | [Handle](#handle) | 04 |
| Verify if an EXE is legitimate | [Sigcheck](#sigcheck) | 04 |
| Pop-ups / browser hijacked | [AdwCleaner](#adwcleaner) | 05 |
| Deeper malware suspected | [ESET Online Scanner](#eset-online-scanner) | 05 |
| Windows won't boot at all | [Medicat](#medicat) | 07 |
| Suspect bad RAM causing crashes | [Medicat → MemTest86](#medicat) | 07 |
...
```mermaid
%%{init: {'theme': 'base', 'flowchart': {'curve': 'basis', 'nodeSpacing': 50, 'rankSpacing': 80}, 'themeVariables': {'primaryColor': '#dbeafe', 'primaryBorderColor': '#3b82f6', 'primaryTextColor': '#1e3a5f', 'background': '#ffffff', 'edgeLabelBackground': '#ffffff'}}}%%
graph TD
    ROOT((💾 DIAG-KIT<br/>114 GB · exFAT))

    subgraph SYS[01 · System Health]
        T01a(HWiNFO64)
        T01b(CPU-Z)
        T01c(Speccy)
    end

    subgraph NET[02 · Network]
        T02a(Angry IP Scanner)
        T02b(CurrPorts)
        T02c(Wireshark)
    end

    subgraph STOR[03 · Storage]
        T03a(CrystalDiskInfo)
        T03b(WinDirStat)
    end

    subgraph INTEG[04 · Integrity]
        T04a(Autoruns)
        T04b(Process Explorer)
        T04c(Process Monitor)
        T04d(TCPView)
        T04e(Sigcheck)
    end

    subgraph MAL[05 · Malware]
        T05a(AdwCleaner)
        T05b(ESET Scanner)
    end

    subgraph SCR[06 · Scripts]
        T06a(".bat Scripts")
    end

    subgraph BOOT[07 · Boot Tools]
        T07a(Medicat PE)
        T07b(MemTest86)
    end
    ...
    NET -. complements .-> INTEG
    INTEG -. escalate if rootkit .-> MAL
    STOR -. failing drive .-> BOOT

    classDef root fill:#fef3c7,stroke:#d97706,color:#78350f,font-weight:bold
    ...
    classDef scr fill:#f3f4f6,stroke:#9ca3af,color:#374151,font-weight:bold
